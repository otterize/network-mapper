...
build-agent: ## Builds the node agent image
	...
	docker buildx build --platform linux/arm64 -t $(OTRZ_AGENT_IMAGE_FULL_NAME) --file build/$(OTRZ_AGENT_IMAGE_NAME).Dockerfile src/

build-mapper: ## Builds the mapper image
	@echo "${PROMPT_COLOR}Building mapper image '$(OTRZ_MAPPER_IMAGE_FULL_NAME)'...${PROMPT_NC}"
	docker buildx build --platform linux/arm64 -t $(OTRZ_MAPPER_IMAGE_FULL_NAME) --file build/$(OTRZ_MAPPER_IMAGE_NAME).Dockerfile src/
...
lima-install-otterize: ## Installs Otterize in the lima kubernetes cluster with the provided client ID and client secret
    ...
    KUBECONFIG=$(LIMA_KUBECONFIG_PATH) && helm upgrade --install \
    	otterize otterize/otterize-kubernetes -n $(OTRZ_NAMESPACE) --create-namespace \
